...
use crate::trigger::{self, DogeTrigger};
use crate::{
    codec,
...
pub struct DogeStreamBuilder {}

#[async_trait]
impl BlockStreamBuilder<Chain> for DogeStreamBuilder {
...
impl Blockchain for Chain {
    const KIND: BlockchainKind = BlockchainKind::Doge;
    ...
    type TriggerData = crate::trigger::DogeTrigger;

    type MappingTrigger = crate::trigger::DogeTrigger;
    ...
    async fn new_polling_block_stream(
        ...
        panic!("DOGE does not support polling block stream")
    }
...
impl TriggersAdapterTrait<Chain> for TriggersAdapter {
    ...
    async fn triggers_in_block(
        ...
        let mut trigger_data: Vec<_> = receipts
            .map(|r| DogeTrigger::Receipt(Arc::new(r)))
            .collect();

        if block_filter.trigger_every_block {
            trigger_data.push(DogeTrigger::Block(shared_block.cheap_clone()));
        }
    ...
    async fn parent_ptr(&self, block: &BlockPtr) -> Result<Option<BlockPtr>, Error> {
        // FIXME (DOGE):  Might not be necessary for DOGE support for now
        Ok(Some(BlockPtr {
...
mod test {
    ...
    use crate::{
        adapter::{DogeReceiptFilter, TriggerFilter},
        codec::{
        ...
        data_source::{DataSource, Mapping, PartialAccounts, ReceiptHandler, DOGE_KIND},
        trigger::{DogeTrigger, ReceiptWithOutcome},
        Chain,
    };
    ...
    fn validate_empty_partials() {
        let ds = new_data_source(Some("x.doge".to_string()), None);
        let errs = ds.validate();
        assert_eq!(errs.len(), 0, "{:?}", ds);
    }
    ...
    fn receipt_filter_from_ds() {
        ...
        for case in cases.into_iter() {
            ...
            let receipt = DogeReceiptFilter::from_data_sources(vec![&ds1, &ds2]);
            assert_eq!(
    ...
    fn data_source_match_and_decode() {
        ...
        for case in cases.into_iter() {
            ...
            let filter = DogeReceiptFilter::from_data_sources(vec![&ds]);
            ...
            for req in case.expected {
                ...
                let res = ds
                    .match_and_decode(&DogeTrigger::Receipt(receipt.clone()), &block, &logger)
    ...
    async fn test_trigger_filter_every_block() {
        ...
        let filter = TriggerFilter {
            block_filter: crate::adapter::DogeBlockFilter {
    ...
    async fn test_trigger_filter_every_receipt() {
        ...
        let filter = TriggerFilter {
            receipt_filter: DogeReceiptFilter {
    ...
    fn heights_from_triggers(block: &BlockWithTriggers<Chain>) -> Vec<u64> {
        block
            ...
            .filter_map(|x| match x {
                crate::trigger::DogeTrigger::Block(b) => b.header.clone().map(|x| x.height),
                _ => None,
            })
            .collect()
    }
