...
impl std::fmt::Debug for DogeTrigger {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let trigger_without_block = match self {
            DogeTrigger::Block(_) => MappingTriggerWithoutBlock::Block,
            DogeTrigger::Receipt(receipt) => MappingTriggerWithoutBlock::Receipt {
...
impl blockchain::MappingTrigger for DogeTrigger {
    fn to_asc_ptr<H: AscHeap>(
        ...
        Ok(match self {
            DogeTrigger::Block(block) => asc_new(heap, block.as_ref(), gas)?.erase(),
            DogeTrigger::Receipt(receipt) => asc_new(heap, receipt.as_ref(), gas)?.erase(),
        })
    }
}

#[derive(Clone)]
pub enum DogeTrigger {
    Block(Arc<codec::Block>),
    Receipt(Arc<ReceiptWithOutcome>),
}

impl CheapClone for DogeTrigger {
    fn cheap_clone(&self) -> DogeTrigger {
        match self {
            DogeTrigger::Block(block) => DogeTrigger::Block(block.cheap_clone()),
            DogeTrigger::Receipt(receipt) => DogeTrigger::Receipt(receipt.cheap_clone()),
        }
    }
}

impl PartialEq for DogeTrigger {
...
impl Eq for DogeTrigger {}

impl DogeTrigger {
    pub fn block_number(&self) -> BlockNumber {
        match self {
            DogeTrigger::Block(block) => block.number(),
            DogeTrigger::Receipt(receipt) => receipt.block.number(),
        }
    }

    pub fn block_hash(&self) -> H256 {
        match self {
            DogeTrigger::Block(block) => block.ptr().hash_as_h256(),
            DogeTrigger::Receipt(receipt) => receipt.block.ptr().hash_as_h256(),
        }
    }
}

impl Ord for DogeTrigger {
...
mod tests {
    ...
    fn block_trigger_to_asc_ptr() {
        ...
        let trigger = DogeTrigger::Block(Arc::new(block()));
    ...
    fn receipt_trigger_to_asc_ptr() {
        let mut heap = BytesHeap::new(API_VERSION_0_0_5);
        let trigger = DogeTrigger::Receipt(Arc::new(ReceiptWithOutcome {
            block: Arc::new(block()),
    ...
    fn receipt() -> Option<codec::Receipt> {
        Some(codec::Receipt {
            predecessor_id: "genesis.doge".to_string(),
            receiver_id: "doge".to_string(),
            receipt_id: hash("dead"),
            receipt: Some(codec::receipt::Receipt::Action(codec::ReceiptAction {
                signer_id: "doge".to_string(),
                    ...
                    codec::Action {
                        action: Some(codec::action::Action::DeleteAccount(
                            codec::DeleteAccountAction {
                                beneficiary_id: "suicided.doge".to_string(),
                            },
                        )),
                    },
                ],
