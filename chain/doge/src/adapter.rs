...
const BASIC_RECEIPT_FILTER_TYPE_URL: &str =
    "type.googleapis.com/sf.doge.transform.v1.BasicReceiptFilter";

#[derive(Clone, Debug, Default)]
pub struct TriggerFilter {
    pub(crate) block_filter: DogeBlockFilter,
    pub(crate) receipt_filter: DogeReceiptFilter,
}
...
impl bc::TriggerFilter<Chain> for TriggerFilter {
    fn extend<'a>(&mut self, data_sources: impl Iterator<Item = &'a DataSource> + Clone) {
        ...
        block_filter.extend(DogeBlockFilter::from_data_sources(data_sources.clone()));
        receipt_filter.extend(DogeReceiptFilter::from_data_sources(data_sources));
    }
...
/// DogeReceiptFilter requires the account to be set, it will match every receipt where `source.account` is the recipient.
/// see docs: https://thegraph.com/docs/en/supported-networks/doge/
#[derive(Clone, Debug, Default)]
pub(crate) struct DogeReceiptFilter {
    pub accounts: HashSet<Account>,
    pub partial_accounts: HashSet<(Option<String>, Option<String>)>,
}

impl DogeReceiptFilter {
    pub fn matches(&self, account: &String) -> bool {
        let DogeReceiptFilter {
            accounts,
            partial_accounts,
        } = self;
    ...
    pub fn extend(&mut self, other: DogeReceiptFilter) {
        let DogeReceiptFilter {
            accounts,
            partial_accounts,
        } = self;
...
/// DogeBlockFilter will match every block regardless of source being set.
/// see docs: https://thegraph.com/docs/en/supported-networks/doge/
#[derive(Clone, Debug, Default)]
pub(crate) struct DogeBlockFilter {
    pub trigger_every_block: bool,
}

impl DogeBlockFilter {
    pub fn from_data_sources<'a>(iter: impl IntoIterator<Item = &'a DataSource>) -> Self {
        Self {
    ...
    pub fn extend(&mut self, other: DogeBlockFilter) {
...
mod test {
    ...
    #[test]
    fn doge_trigger_empty_filter() {
        let filter = TriggerFilter {
            block_filter: DogeBlockFilter {
                trigger_every_block: false,
            },
            receipt_filter: super::DogeReceiptFilter {
    ...
    #[test]
    fn doge_trigger_filter_match_all_block() {
        let filter = TriggerFilter {
            block_filter: DogeBlockFilter {
    ...
    #[test]
    fn doge_trigger_partial_filter() {
        let filter = TriggerFilter {
            block_filter: DogeBlockFilter {
                trigger_every_block: false,
            },
